...
export default function EventDetailClient({ category, details }: Props) {
  ...
  const prizeData = prizeEntry
    ? {
        first: prizeEntry.prizeFirst,
        second: prizeEntry.prizeSecond ?? 0,
        third: prizeEntry.prizeThird ?? 0,
      }
    : null;
  const pointsData = prizeEntry
    ? {
        first: prizeEntry.pointsFirst,
        second: prizeEntry.pointsSecond ?? 0,
        third: prizeEntry.pointsThird ?? 0,
      }
    : null;
  const showPrizeCard = Boolean(
    prizeData && (prizeData.first || prizeData.second || prizeData.third),
  );
  const showPointsCard = Boolean(
    pointsData && (pointsData.first || pointsData.second || pointsData.third),
  );
  const hasSecondPrize = (prizeData?.second ?? 0) > 0;
  const hasThirdPrize = (prizeData?.third ?? 0) > 0;
  const totalPrizePool = prizeData
    ? prizeData.first +
      (hasSecondPrize ? prizeData.second : 0) +
      (hasThirdPrize ? prizeData.third : 0)
    : 0;
  const detailsColSpan =
    showPrizeCard || showPointsCard ? "lg:col-span-2" : "lg:col-span-3";
  const formatPrizeAmount = (value?: number) =>
    value && value > 0 ? `₹${value.toLocaleString("en-IN")}` : "TBD";
    ...
    <div className="min-h-screen bg-gat-off-white font-body pt-24 pb-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {(showPrizeCard || showPointsCard) && (
            <div className="lg:col-span-1">
              <div className="sticky top-28 space-y-4">
                {showPrizeCard && (
                  <div className="rounded-2xl border border-gat-blue/10 bg-white p-5 shadow-sm">
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-3 rounded-lg border border-yellow-300/40 bg-gradient-to-r from-yellow-50 to-amber-50">
                        ...
                        <div className="font-mono font-black text-base text-amber-700">
                          {formatPrizeAmount(prizeData?.first)}
                        </div>
                      </div>
                      {hasSecondPrize && (
                        <div className="flex items-center justify-between p-3 rounded-lg border border-gray-300/40 bg-gradient-to-r from-gray-50 to-slate-50">
                          ...
                          <div className="font-mono font-black text-base text-slate-600">
                            {formatPrizeAmount(prizeData?.second)}
                          </div>
                        </div>
                      )}
                      {hasThirdPrize && (
                        <div className="flex items-center justify-between p-3 rounded-lg border border-orange-300/40 bg-gradient-to-r from-orange-50 to-amber-50/50">
                          ...
                          <div className="font-mono font-black text-base text-orange-700">
                            {formatPrizeAmount(prizeData?.third)}
                          </div>
                        </div>
                      )}
                      <div className="mt-2 pt-3 border-t border-gat-steel/10 flex items-center justify-between">
                        ...
                        <span className="font-mono font-black text-gat-blue text-base">
                          {formatPrizeAmount(totalPrizePool)}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                {showPointsCard && (
                  <div className="rounded-2xl border border-gat-blue/10 bg-white p-5 shadow-sm">
                    <h3 className="flex items-center gap-2 text-lg font-bold font-heading text-gat-midnight mb-4">
                      <Trophy className="w-5 h-5 text-gat-blue" />
                      Rolling Trophy Points
                    </h3>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-3 rounded-lg border border-blue-200/40 bg-gradient-to-r from-blue-50 to-slate-50">
                        <div className="flex items-center gap-3">
                          <span className="text-2xl">🥇</span>
                          <div>
                            <p className="font-bold text-gat-midnight text-sm">
                        ...
                        <div className="font-mono font-black text-base text-gat-blue">
                          {pointsData?.first ?? 0}
                        </div>
                      </div>
                      {pointsData?.second ? (
          ...
          <div className={`${detailsColSpan} space-y-6`}>
